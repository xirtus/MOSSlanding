...
# ── Generate app icon (native Swift, no Python) ──────────────────────────────
echo ""
...
if command -v swift &>/dev/null; then
    swift "$SCRIPT_DIR/scripts/make_icon.swift" "$ICONSET" && \
        iconutil -c icns "$ICONSET" -o "$RESOURCES/AppIcon.icns" && \
        echo "  AppIcon.icns created (CoreGraphics)" || echo "  WARNING: icon generation failed"
else
    echo "  Skipping icon (install Xcode Command Line Tools to enable)"
fi
