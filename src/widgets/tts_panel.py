...
class ModelLoadWorker(QThread):
    """Loads a model in a background thread so the UI stays responsive."""
...
class TTSPanel(QWidget):
    ...
    def __init__(self, backend, dark_mode: bool = True):
        ...
        self._pending_generation: bool = False  # generate after model loads
    ...
    def _setup_ui(self):
        ...
        # Quality selector (RVQ codebook layers)
        q_row = QHBoxLayout()
        q_label = QLabel("Quality")
        q_label.setStyleSheet("font-weight: 600; font-size: 12px;")
        q_row.addWidget(q_label)

        self._quality_btns: list[QPushButton] = []
        self._quality_values = [4, 8, 16, 24, 32]
        self._quality = 32  # default: max fidelity
        quality_hints = {4: "Fastest", 8: "Fast", 16: "Balanced", 24: "High", 32: "Best ★"}
        for qv in self._quality_values:
            btn = QPushButton(f"{qv}×  {quality_hints.get(qv, '')}")
            btn.setCheckable(True)
            btn.setChecked(qv == self._quality)
            btn.setMinimumWidth(72)
            btn.setFixedHeight(28)
            btn.setCursor(Qt.PointingHandCursor)
            btn.setToolTip(f"{quality_hints.get(qv, '')} — {qv} RVQ codebook layers")
            btn.clicked.connect(lambda checked, v=qv: self._set_quality(v))
    ...
    def _set_quality(self, value: int):
        """Set the RVQ codebook layer count (4, 8, 16, or 32)."""
        self._quality = value
        for btn, qv in zip(self._quality_btns, self._quality_values):
            btn.setChecked(qv == value)
            # Style active button distinctively
            if qv == value:
                btn.setStyleSheet(
                    "QPushButton { background-color: rgba(64,200,224,0.15); "
                    "color: #40c8e0; border: 1px solid #40c8e0; font-weight: 600; }"
                )
            else:
                btn.setStyleSheet("")
    ...
    def _do_load_then_generate(self):
        """Load the model in background, then auto-generate."""
        self._pending_generation = True
        self._generate_btn.setEnabled(False)
        self._progress.setVisible(True)
        self._output_status.setText("Loading model …")

        from src.backend import MODEL_TTS
        self._loader = ModelLoadWorker(self._backend, MODEL_TTS)
        self._loader.progress.connect(lambda msg: self._output_status.setText(msg))
    ...
    def _on_load_for_generate_error(self, error: str):
        ...
        self._generate_btn.setEnabled(True)
        self._progress.setVisible(False)
        self._output_status.setText(f"✗ Load error: {error}")
        QMessageBox.critical(self, "Load Error", error)

    def _start_inference(self):
        """Launch TTS inference (model must already be loaded)."""
        text = self._text_edit.toPlainText().strip()
